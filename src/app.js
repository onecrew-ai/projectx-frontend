 ...
 * Accounts for ROS2 resolution, origin (x,y), and yaw rotation.
 */
function pixelToMetres(px, py) {
  const { resolution, originX, originY, originYaw, imageHeight } = STATE.map;

  // 1. Calculate pixel distance from the lower-left corner (ROS origin point)
  const dxPx = px;
  const dyPx = imageHeight - py;

  // 2. Convert to unrotated meters
  const mxUnrotated = dxPx * resolution;
  const myUnrotated = dyPx * resolution;

  // 3. Apply yaw rotation
  const cosYaw = Math.cos(originYaw);
  const sinYaw = Math.sin(originYaw);

  const mxRotated = mxUnrotated * cosYaw - myUnrotated * sinYaw;
...
function metresToPixel(mx, my) {
  ...
  // 2. Rotate back by negative yaw to get unrotated meters
  // Note: cos(-a) = cos(a), sin(-a) = -sin(a)
  const cosYaw = Math.cos(originYaw);
  const sinYaw = Math.sin(originYaw);

  const mxUnrotated = dx * cosYaw + dy * sinYaw;
  const myUnrotated = -dx * sinYaw + dy * cosYaw;

  // 3. Convert to pixels from lower-left
  const px = mxUnrotated / resolution;
  const pyFromBottom = myUnrotated / resolution;

  // 4. Map back to canvas coordinates (invert Y back to top-down)
  const py = imageHeight - pyFromBottom;
...
function render() {
  ...
  if (STATE.currentTool === 'dense' && STATE.densePath.p1 && STATE.hover.wx !== null) {
    drawDensePreview();
  }
...
function drawDensePreview() {
  ...
  const { cx, cy } = worldToCanvas(p2.wx, p2.wy);
  const nearest = nearestWaypoint(cx, cy);

  if (nearest && nearest.distPx <= STATE.snapThreshold) {
    const snapWp = STATE.waypoints.get(nearest.wpId);
    if (snapWp) {
      p2 = { wx: snapWp.x, wy: snapWp.y };
      isSnapped = true;
    }
  }
  ...
  // FIXED: Calculate steps purely in world coordinates
  ...
  for (let i = 1; i < steps; i++) { 
    ...
    const y = p1.wy + dy * i;
    ctx.beginPath();
    ctx.arc(x, y, 3 / STATE.view.zoom, 0, Math.PI * 2);
    ctx.fill();
  }
...
/** Dense path: interpolate waypoints between two world-space positions
 * at fixed intervals (STATE.denseInterval), linking them as a chain.
 */
function createDensePath(wx1, wy1, wx2, wy2) {
  ...
  showToast(`Waypoints: ${steps} segments created`);
}
...
function handleEventToggle(cx, cy) {
  ...
  if (!STATE.activeEventId) {
    showToast('Create or select an Event in the Events tab first.');
    return;
  }
  ...
  if (wp.eventId === STATE.activeEventId) {
    wp.eventId = null;
    showToast(`Event removed from W${wp.id}`);
  } else {
...
function handleDenseClick(wx, wy) {
  if (!STATE.densePath.p1) {
    ...
    showToast('Waypoints: P1 set — click P2');
    scheduleRender();
  } else {
...
function updateInspector() {
  ...
  if (STATE.selectedWpId !== null) {
    ...
    if (wp.neighbors.size === 0) {
      chipsEl.innerHTML = '<span style="font-family:var(--font-mono);font-size:13px;color:var(--text-dim);">No connections</span>';
    }
...
document.getElementById('wp-label-input').addEventListener('input', e => {
  ...
  const wp = STATE.waypoints.get(STATE.selectedWpId);
  if (wp) { wp.label = e.target.value; scheduleRender(); }
});
...
function renderEventsPanel() {
  ...
  // 1. Top Control: Create New Event
  const topCtrl = document.createElement('div');
  topCtrl.innerHTML = `<button class="hbtn primary" id="btn-create-event" style="width:100%; margin-bottom:12px;">+ CREATE NEW EVENT</button>`;
  listEl.appendChild(topCtrl);
  ...
  STATE.events.forEach(event => {
    // Find all waypoints that have this event assigned
    const assignedWps = [];
    STATE.waypoints.forEach(wp => { if (wp.eventId === event.id) assignedWps.push(wp.id); });

    const isActive = STATE.activeEventId === event.id;
    const card = document.createElement('div');
    card.className = 'event-card';
    if (isActive) card.style.borderColor = 'var(--accent-green)';

    card.innerHTML = `
      <div class="event-card-header btn-activate-event" data-id="${event.id}" style="background: ${isActive ? 'rgba(0,255,136,0.1)' : 'rgba(255,170,0,0.08)'}; cursor: pointer; transition: background 0.2s;">
        <div style="display:flex; align-items:center; gap: 8px; flex: 1;">
          <span style="color: ${isActive ? 'var(--accent-green)' : 'var(--text-dim)'}; font-size: 17px;">${isActive ? '●' : '○'}</span>
          <input type="text" class="prop-input ev-name-input" data-id="${event.id}" value="${escapeHTML(event.name)}" placeholder="Event Name" style="width: 140px; border-color: transparent; background: rgba(0,0,0,0.2); pointer-events: auto;" />
        </div>
        <button class="event-remove-btn" data-id="${event.id}" title="Delete event entirely">✕</button>
      </div>
      
      <div style="padding: 4px 10px; font-size: 12px; color: var(--text-dim); border-bottom: 1px solid var(--border-dim); background: var(--bg-panel);">
        Anchored to: ${assignedWps.length ? assignedWps.map(id => `<span class="neighbor-chip" style="border-color:var(--accent-amber); color:var(--accent-amber);">W${id}</span>`).join('') : 'None'}
      </div>

      <div class="trigger-section">
        <div class="trigger-label">▶ On Entry</div>
        <ul class="action-list">
          ${renderActionItems(event.onEntry, event.id, 'entry')}
        </ul>
        <div class="add-action-row">
          <select id="entry-type-${event.id}">
            ${Object.keys(ACTION_TYPES).map(k => `<option value="${k}">${k}</option>`).join('')}
          </select>
          <input type="text" id="entry-param-${event.id}" placeholder="param" />
          <button class="add-action-btn" data-evid="${event.id}" data-trigger="entry">+</button>
        </div>
      </div>
      ...
      <div class="trigger-section" style="border-bottom:none;">
        ...
        <ul class="action-list">
          ${renderActionItems(event.onExit, event.id, 'exit')}
        </ul>
        <div class="add-action-row">
          <select id="exit-type-${event.id}">
            ${Object.keys(ACTION_TYPES).map(k => `<option value="${k}">${k}</option>`).join('')}
          </select>
          <input type="text" id="exit-param-${event.id}" placeholder="param" />
          <button class="add-action-btn" data-evid="${event.id}" data-trigger="exit">+</button>
        </div>
      </div>
    `;
...
async function exportToDb() {
  ...
  db.run(`
    ...
    CREATE TABLE IF NOT EXISTS map_meta (key TEXT PRIMARY KEY, value TEXT);
    CREATE TABLE IF NOT EXISTS waypoints (id TEXT PRIMARY KEY, x REAL, y REAL, label TEXT, event_id TEXT);
    CREATE TABLE IF NOT EXISTS waypoint_neighbors (waypoint_id TEXT, neighbor_id TEXT);
    CREATE TABLE IF NOT EXISTS edges (id TEXT PRIMARY KEY, from_wp TEXT, to_wp TEXT, direction TEXT, speed_limit REAL, lane_width REAL);
    CREATE TABLE IF NOT EXISTS event_defs (id TEXT PRIMARY KEY, name TEXT);
    CREATE TABLE IF NOT EXISTS event_actions (event_id TEXT, trigger TEXT, action_type TEXT, action_param TEXT, action_order INTEGER);
  `);
  ...
  if (STATE.map.image && STATE.map.image.src) {
    mapM.image_data_url = STATE.map.image.src;
  }
  ...
  // ── Insert waypoints ───────────────────────────────────────────────────────
  STATE.waypoints.forEach(wp => {
    db.run('INSERT INTO waypoints VALUES (?, ?, ?, ?, ?)', [wp.id, wp.x, wp.y, wp.label || '', wp.eventId || '']);
    wp.neighbors.forEach(nid => { db.run('INSERT INTO waypoint_neighbors VALUES (?, ?)', [wp.id, nid]); });
  });
  ...
  STATE.events.forEach(evt => {
    db.run('INSERT INTO event_defs VALUES (?, ?)', [evt.id, evt.name]);
    const insertActions = (actions, trigger) => {
      actions.forEach((action, order) => {
        db.run('INSERT INTO event_actions VALUES (?,?,?,?,?)', [evt.id, trigger, action.type, action.param || '', order]);
      });
    };
...
/** Generate an Occupancy Grid from Edges and export as PGM + YAML */
function exportOccupancyGrid() {
  if (STATE.edges.size === 0) {
    showToast('No edges to generate grid from.');
    return;
  }
  ...
  // 3. Fill with "Occupied" (Black / 0)
  ctx.fillStyle = '#000000';
  ctx.fillRect(0, 0, width, height);

  // 4. Draw Lanes as "Free" (White / 255)
  ...
  STATE.edges.forEach(edge => {
    ...
    // FIX: Waypoints are already in image pixel coordinates, 
    // so we can use a.x and a.y directly!
    const pxA = a.x;
    const pyA = a.y;
    const pxB = b.x;
    const pyB = b.y;
  ...
  const pgmHeader = `P5\n${width} ${height}\n255\n`;
  const headerBlob = new Blob([pgmHeader], { type: 'text/plain' });
  const pixelBlob = new Blob([pixelArray], { type: 'application/octet-stream' });
  const finalPgmBlob = new Blob([headerBlob, pixelBlob]);

  // 7. Build ROS2 YAML Companion Blob
  const yamlText = `image: occupancy_grid.pgm
resolution: ${res}
origin: [${STATE.map.originX}, ${STATE.map.originY}, ${STATE.map.originYaw}]
negate: 0
...
async function importFromDb(file) {
  if (typeof initSqlJs === 'undefined') {
    showToast('sql.js not loaded — check internet connection'); return;
  }
  ...
  // ── Validate schema ────────────────────────────────────────────────────────
  const tables = ['project_meta', 'waypoints', 'edges'];
  ...
  if (importedImageDataUrl) {
    await new Promise(resolve => {
      ...
      img.src = importedImageDataUrl;
    });
  }

  // ── Load waypoints (Dynamic check for backward compatibility) ──────────────
  const wpRows = db.exec('SELECT * FROM waypoints');
  if (wpRows.length) {
  ...
  if (isNewSchema) {
    ...
    if (defRows.length) {
      defRows[0].values.forEach(([id, name]) => {
        STATE.events.set(id, { id, name, onEntry: [], onExit: [] });
        const idNum = parseInt(id.replace('E', ''));
        if (!isNaN(idNum) && idNum >= STATE.nextEventId) STATE.nextEventId = idNum + 1;
      });
    }
    const actRows = db.exec('SELECT event_id, trigger, action_type, action_param FROM event_actions ORDER BY event_id, trigger, action_order');
    if (actRows.length) {
      actRows[0].values.forEach(([evId, trig, atype, aparam]) => {
        const evt = STATE.events.get(evId);
        if (evt) {
    ...
    // Legacy Import: Convert old waypoint-based events into reusable events
    const evtRows = db.exec('SELECT waypoint_id, trigger, action_type, action_param FROM events ORDER BY waypoint_id, trigger, action_order');
    if (evtRows.length) {
      evtRows[0].values.forEach(([wpId, trig, atype, aparam]) => {
        const evId = 'E' + wpId; // Generate pseudo-ID based on waypoint
        if (!STATE.events.has(evId)) {
          STATE.events.set(evId, { id: evId, name: `Legacy Event ${wpId}`, onEntry: [], onExit: [] });
          const wp = STATE.waypoints.get(wpId);
          if (wp) wp.eventId = evId;
        }
        const evt = STATE.events.get(evId);
        const arr = trig === 'entry' ? evt.onEntry : evt.onExit;
        arr.push({ type: atype, param: aparam || '' });
...
document.getElementById('btn-export-grid').addEventListener('click', exportOccupancyGrid);
...
document.getElementById('events-list').addEventListener('click', (e) => {
  // 1. Create New Event
  if (e.target.id === 'btn-create-event') {
    const id = 'E' + (STATE.nextEventId++);
    STATE.events.set(id, { id, name: `Sequence ${id}`, onEntry: [], onExit: [] });
    STATE.activeEventId = id; // Auto-activate new event
    setTool('event'); // Auto-switch to event tool
    renderEventsPanel();
    scheduleRender();
    return;
  }

  // 2. Set Event as Active (Clicking the header)
  const header = e.target.closest('.btn-activate-event');
  if (header && !e.target.matches('.ev-name-input') && !e.target.matches('.event-remove-btn')) {
    STATE.activeEventId = header.dataset.id;
    setTool('event');
    renderEventsPanel();
    return;
  }

  // 3. Delete Event entirely
  if (e.target.matches('.event-remove-btn')) {
    const evId = e.target.dataset.id;
    STATE.events.delete(evId);
    if (STATE.activeEventId === evId) STATE.activeEventId = null;
    // Strip this event from any waypoints that had it
    STATE.waypoints.forEach(wp => { if (wp.eventId === evId) wp.eventId = null; });
    renderEventsPanel();
    scheduleRender();
    return;
  }
  ...
  // 5. Remove Action
  if (e.target.matches('.action-del')) {
    const { wpid, trig, idx } = e.target.dataset;
    const event = STATE.events.get(wpid); // Note: dataset still says wpid from renderActionItems, but it's passing evId now
    if (event) {
...
// Update event names on change (so it doesn't lose focus while typing)
document.getElementById('events-list').addEventListener('change', (e) => {
  if (e.target.matches('.ev-name-input')) {
    const evId = e.target.dataset.id;
    const event = STATE.events.get(evId);
    if (event) event.name = e.target.value;
  }
});
